...
import { TABS_LIST, ordersList } from "../../Components/Pages/Orders/const";
...
const ordersSlice = createSlice({
  ...
  reducers: {
    checkedForRemove(state, action) {
      state.data = state.data?.map(item => item?.id === action?.payload?.id ? {...item, checked: true} : item);
    },
    ...
    searchStatusHandler(state, action) {
      state.data = ordersList.filter(item => action.payload === 'all' ? item : item.status.toLowerCase() === action.payload.toLowerCase())
    },
    searchPaymentHandler(state, action) {
      state.data = ordersList.filter(item => !action.payload?.value ? item : item?.payment_status?.toLowerCase()?.trim() === action.payload?.value?.toLowerCase()?.trim())
    },
